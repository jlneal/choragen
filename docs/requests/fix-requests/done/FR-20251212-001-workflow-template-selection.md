...
Fixed by changing `loadTemplate(ctx.projectRoot, "standard")` to `loadTemplate(ctx.projectRoot, input.template)` in `packages/web/src/server/routers/workflow.ts`. Also updated the JSDoc comment to reflect the fix.
